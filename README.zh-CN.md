...
> **核心要点**：LLM 永远看不到你的真实数据。它处理的是*看起来真实*的脱敏值。当响应返回 OpenCode 时，Guard 会将脱敏值还原为原始值——但前提是格式被保留了。如果 LLM 对值进行了变换（例如 `@` → `_AT_`），还原就不可能了，这恰恰证明了 LLM 自始至终只拥有脱敏版本。
