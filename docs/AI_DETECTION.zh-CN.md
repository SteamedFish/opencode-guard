...
当模型标记的某个值（例如邮箱）在同一条消息中出现多次时，插件会 mask **该值在该文本中所有「整 token」出现位置**，而不只是第一个，否则其余副本会以明文到达 provider。嵌入在更长 ASCII 标识符内部的出现位置（`John` 在 `Johnson` 中、`1234` 在 `ref1234x` 中）不会扩展，因为它属于另一个 token；模型实际检出的那个 span 始终会被 mask。

mask 仍然保持确定性：同一个值的每一处出现都映射为同一个 masked token，因此响应还原不受影响。
